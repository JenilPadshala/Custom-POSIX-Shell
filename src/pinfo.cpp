...
#include "../include/prompt.h" // For resolving '~' with shell_home
#include <cstdio>
#include <cstring>
#include <cstdlib>
#include <unistd.h>
...
void execute_pinfo(char** args, int arg_count) {
    ...
    if (arg_count == 1) {
        target_pid = getpid(); // Current shell PID
    } else if (arg_count == 2) {
        char* endptr = nullptr;
    ...
    // 2. Fetch Process Status & Process Group Info
    ...
    char status_char = 'S';
    // Status definitions: 1 = SIDL, 2 = SRUN, 3 = SSLEEP, 4 = SSTOP, 5 = SZOMB
    ...
    // 5. Display the process information
    std::printf("pid -- %d\n", target_pid);
